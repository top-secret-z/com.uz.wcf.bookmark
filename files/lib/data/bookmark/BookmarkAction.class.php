...
class BookmarkAction extends AbstractDatabaseObjectAction {
	...
	public function addBookmark() {
		...
		$data = [
				...
				'objectID' => isset($this->parameters['objectID']) ? $this->parameters['objectID'] : 0,
				'url' => $this->parameters['url'],
				'userID' => $user->userID,
				'username' => $user->username
		];
		
		$action = new BookmarkAction([], 'create', ['data' => $data]);
	...
	public function load() {
		...
		if (WCF::getUser()->userID == $this->parameters['userID']) {
			if ($this->parameters['bookmarkAccess'] == 'public') $bookmarkList->getConditionBuilder()->add("isPrivate = ?", [0]);
			if ($this->parameters['bookmarkAccess'] == 'private') $bookmarkList->getConditionBuilder()->add("isPrivate = ?", [1]);
			if ($this->parameters['bookmarkAccess'] == 'follower') $bookmarkList->getConditionBuilder()->add("isPrivate = ?", [2]);
		}
		else {
			// follower?
	...
	public function deleteBookmark() {
		$action = new BookmarkAction([$this->bookmark], 'delete');
	...
	public function shareBookmark() {
		...
		foreach ($receiverList as $user) {
			if (!$user->getPermission('user.bookmark.canUseBookmark')) continue;
		...
		if (count($userIDs)) {
			...
			$action = new BookmarkAction([$this->bookmark], 'update', ['data' => $data]);
			$action->executeAction();
		}
